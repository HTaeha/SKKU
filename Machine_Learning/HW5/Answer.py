...
def convolution2d(x, kernel, stride):
    ...
    conv_out = np.zeros(((height-kernel_size)//stride+1, (width-kernel_size)//stride+1))
        ...
        for j in range(len(conv_out[0])):
            for k_i in range(kernel_size):
                for k_j in range(kernel_size):
                    conv_out[i][j] += kernel[k_i][k_j] * x[k_i+stride*n][k_j+stride*m]
            m += 1
        m = 0
        n += 1 
...
class ReLU:
    ...
    def forward(self, z):
        ...
        self.zero_mask = np.zeros(z.shape)
        index = np.where(z>0)
        out[index] = z[index]
        self.zero_mask[np.where(z<=0)] = 1
        # =========================================================================
        return out
...
class MaxPoolingLayer:
    ...
    def forward(self, x):
        ...
        # =============================== EDIT HERE ===============================
        pool_height = height//self.kernel_size
        pool_width = width//self.kernel_size
            ...
            for j in range(channel):
                for p_h in range(pool_height):
                    for p_w in range(pool_width):
                        a, b = np.unravel_index(x[i,j,p_h*self.kernel_size:(p_h+1)*(self.kernel_size),p_w*self.kernel_size:(p_w+1)*(self.kernel_size)].argmax(), x[i,j,p_h*self.kernel_size:(p_h+1)*(self.kernel_size),p_w*self.kernel_size:(p_w+1)*(self.kernel_size)].shape)
                        self.mask[i,j,a+p_h*self.kernel_size,b+p_w*self.kernel_size] = 1
                        max_pool[i,j,p_h,p_w] = np.max(x[i,j,p_h*self.kernel_size:(p_h+1)*(self.kernel_size), p_w*self.kernel_size:(p_w+1)*(self.kernel_size)])
        # =========================================================================
    ...
    def backward(self, d_prev=1):
        ...
        # =============================== EDIT HERE ===============================
        d_max = np.array(self.mask)
        for i in range(batch):
            for j in range(channel):
                for h in range(height):
                    for w in range(width):
                        a, b = np.where(self.mask[i,j,h*self.kernel_size:(h+1)*self.kernel_size,w*self.kernel_size:(w+1)*self.kernel_size]==1)
                        d_max[i,j,a+h*self.kernel_size,b+w*self.kernel_size] *= d_prev[i,j,h,w]
        # =========================================================================
        return d_max
...
class FCLayer:
    ...
    def forward(self, x):
        ...
        self.x = x
        # =============================== EDIT HERE ===============================
        self.out = self.x.dot(self.W) + self.b
        # =========================================================================
        return self.out
...
class SoftmaxLayer:
    ...
    def backward(self, d_prev=1):
        ...
        dx = None
        # =============================== EDIT HERE ===============================
        dx = (self.y_hat - self.y)/len(self.y)
        # =========================================================================
        return dx
    ...
    def ce_loss(self, y_hat, y):
        ...
        self.y = y
        # =============================== EDIT HERE ===============================
        self.loss = -np.sum(y*np.log(y_hat + eps))
        self.loss /= len(y)
        # =========================================================================
        return self.loss

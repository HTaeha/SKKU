...
def sign(z):
    ...
    sign_z = np.zeros(z.shape)
    sign_z[np.where(z>=0)] = 1
    sign_z[np.where(z<0)] = -1
...
class SigmoidOutputLayer:
    ...
    def binary_ce_loss(self, y_hat, y):
        ...
        bce_loss = None
        # =============== EDIT HERE ===============
        bce_loss = -np.sum(y*np.log(y_hat + eps) + (1-y)*np.log(1-y_hat + eps))
        bce_loss /= len(y)
        # =========================================
        return bce_loss
    ...
    def backward(self, d_prev=1):
        ...
        dx = None
        # =============== EDIT HERE ===============
        temp = (self.y_hat - self.y)/len(self.y)
        dx = temp.dot(self.W.T)
        self.dW = self.x.T.dot(temp)
        self.db = temp.sum(axis=0)
        # =========================================

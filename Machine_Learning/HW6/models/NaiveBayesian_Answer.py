            ...
            if idx in self.Category_feature_idx:
                ...
                for attr, cnt in zip(attribute, count):
                  # to avoid key Error in predict fuction.
                    ...
                    likelihood['%s = %s' %(feature_name, value)] = round((cnt+1)/(len(column_data)+len(attribute)), 6)
            else:
                feature_name = f
                likelihood['%s_mean' %(feature_name)] = np.mean(data[f].values)
                likelihood['%s_std' %(feature_name)] = np.std(data[f].values)
        # ================================================
        return

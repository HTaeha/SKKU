    ...
    def Find_Best_Feature(self, df):
        ...
        # ============       Edit here      ==================
            ...
            if idx in self.Category_feature_idx:
                impurity = 0
                temp = dict()
                for i, item in enumerate(df[h].values):
                    if item in temp:
                        temp[item] += 1
                    else:
                        temp[item] = 1
                ...
                for key in temp:
                    ...
                    val = impurity_func(temp_y, self.criterion)
                    impurity += val*len(temp_y)/len(df[h].values)
            ...
            # Numeric Feature Case
                ...
                for i, item in enumerate(df[h].values):
                    if item < split_value:
                        temp_y_s.append(Y_data[i])
                    else:
                        temp_y_l.append(Y_data[i])
                val_s = impurity_func(temp_y_s, self.criterion)
                val_l = impurity_func(temp_y_l, self.criterion)
                impurity = val_s*len(temp_y_s)/len(df[h].values) + val_l*len(temp_y_l)/len(df[h].values)

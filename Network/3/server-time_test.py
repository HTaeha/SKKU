    ...
    if file_name == '/' and file_split[0] == 'GET':
        ...
        if file_split[0] == 'POST':
            ...
            with open('secret.html', 'rb') as f:
                ...
                connectionSocket.send('HTTP/1.1 200 OK\r\nSet-Cookie: id_cookie={}; max-age=30\r\nSet-Cookie: passwd_cookie={}; max-age=30\r\nSet-Cookie: time={}; max-age=30\r\nContent-Type: text/html\r\nContent-Length:{}\r\n\r\n'.format(id_str,passwd_str,time.time(), file_size).encode())
                while True:
                    buffer = f.read(chunk_size)
                    if not buffer: break
                    connectionSocket.send(buffer)
            ...
            if file_name == 'cookie.html':
                if cookie_flag == 1:
                    last = int(time.time() - tm)
                    print(last)
                    connectionSocket.send(b'HTTP/1.1 200 OK\r\nContent-Type: text/html\r\n\r\n')
                    connectionSocket.send(('<html><body><h1>Hello '+ID+'</h1><br><br><h1>'+last+'seconds left until your cookie expires.</h1></body></html>').encode())
                else:
